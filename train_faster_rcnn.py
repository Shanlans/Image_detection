...
def main(argv):
    ...
    tensorboard = FasterRcnnTensorboard(log_dir='./logs', histogram_freq=1, validation_data=valide_gen,
                                        batch_size=FLAGS.batch_size, write_images=True, update_freq='batch')

    hist = model_rpn.fit_generator(train_gen, steps_per_epoch=len(train_gen), epochs=FLAGS.epochs,
                                   validation_data=valide_gen, validation_steps=len(valide_gen),
